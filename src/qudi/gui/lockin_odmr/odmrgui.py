...
class OdmrGui(GuiBase):
    ...
    odmr_gui:
        ...
        connect:
            odmr_lockinlogic: 'odmr_lockinlogic'
    """

    # declare connectors
    _odmr_lockinlogic = Connector(name='odmr_lockinlogic', interface='OdmrLockinLogic')
    ...
    def __connect_cw_control_signals(self):
        self._cw_control_dockwidget.sigCwParametersChanged.connect(
            self._odmr_lockinlogic().set_cw_parameters
        )
    ...
    def _apply_odmr_settings(self):
        self._odmr_lockinlogic().set_sample_rate(
    ...
    def _update_scan_state(self, running=None):
        ...
        """
        if running is None:
            running = self._odmr_lockinlogic().module_state() != 'idle'
        # set controls state
    ...
    def _update_cw_parameters(self, parameters=None):
        if parameters is None:
            parameters = self._odmr_lockinlogic().cw_parameters
